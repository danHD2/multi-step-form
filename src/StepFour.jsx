...
function service(everything) {
    ...
    if (everything.service) {
        {everything.monthly ? total += yearlyPrices.service : total += monthlyPrices.service}
        return <div className="mb-3 flex justify-between">
            <p className="text-coolGray">Online service</p>
            <p className="text-marineBlue">{`${everything.monthly ? `+$${yearlyPrices.service}/yr` : `+$${monthlyPrices.service}/mo`}`}</p>
        </div>
    }; 
...
function storage(everything) {
    if (everything.storage) {
        {everything.monthly ? total += yearlyPrices.storage : total += monthlyPrices.storage}
        return <div className="mb-3 flex justify-between">
                 <p className="text-coolGray">Larger storage</p>
                 <p className="text-marineBlue">{`${everything.monthly ? `+$${yearlyPrices.storage}/yr` : `+$${monthlyPrices.storage}/mo`}`}</p>
             </div>
    }
}
...
function profile(everything) {
    if (everything.profile) {
        {everything.monthly ? total += yearlyPrices.profile : total += monthlyPrices.profile}
        return <div className="mb-3 flex justify-between">
                 <p className="text-coolGray">Customizable profile</p>
                 <p className="text-marineBlue">{`${everything.monthly ? `+$${yearlyPrices.profile}/yr` : `+$${monthlyPrices.profile}/mo`}`}</p>
             </div>
    }
}
...
return (
    <div>{everything.success ? <StepFive /> : 
        ...
        <div className="bg-alabaster rounded-lg mt-8 p-5">
            <div className="flex justify-between items-center">
                <div>
                    <h2 className="font-bold text-marineBlue">{everything.plan} {everything.monthly ? "(Yearly)" : "(Monthly)"}</h2>
                    <p onClick={() => {setEverything({...everything, chapter: 2})}} className="text-coolGray underline cursor-pointer">Change</p>
                </div>
                <p className="font-bold text-marineBlue">{`$${identifyPlan(everything)}`}</p>
            </div>
        ...
        <div className="p-5 flex justify-between">
            ...
            <p className="text-lg font-bold text-purplishBlue">{`$${total}/${everything.monthly ? "yr" : "mo"}`}</p>
        </div>
    </div>
    }</div>
)
